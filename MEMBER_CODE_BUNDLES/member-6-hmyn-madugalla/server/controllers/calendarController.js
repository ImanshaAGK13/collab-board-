...
export async function getEvents(req, res) {
  try {
    ...
    if (events && events.length > 0) return res.status(200).json({ success: true, events });
  } catch {}
...
export async function addEvent(req, res) {
  try {
    ...
    try {
      const created = await Event.create(newEv);
      return res.status(201).json({ success: true, event: created });
    } catch {
      inMemoryEvents.push({ id: newEv.eventId, ...newEv });
